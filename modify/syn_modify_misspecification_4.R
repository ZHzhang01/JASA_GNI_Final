...
X <- as.matrix(data1[, -1])
G_true <- as.matrix(data2[, -1])
E_true <- apply(G_true, c(1, 2), function(x) ifelse(x != 0, 1, 0))
num_nb_true <- rowSums(E_true)
...
flip_adj_symmetric <- function(E, p = 0.005, seed = NULL) {
  if (!is.matrix(E) || nrow(E) != ncol(E)) stop("A square matrix is required")
  if (!is.null(seed)) set.seed(seed)
  n <- nrow(E)
  E <- matrix(as.integer(E != 0), n, n)
  
  flip_upper <- matrix(FALSE, n, n)
  idx <- upper.tri(E, diag = FALSE)
  flip_upper[idx] <- runif(sum(idx)) < p
  flip_mask <- flip_upper | t(flip_upper)
  
  E_new <- (E + flip_mask) %% 2
  diag(E_new) <- 0L
  storage.mode(E_new) <- "integer"
  return(E_new)
}
E <- flip_adj_symmetric(E_true, p = 0.0004)
G <- E/rowSums(E)
G <- as.matrix(G)
num_nb <- rowSums(E)
...
get_Y <- function(Z){
  ...
  Y_0 <- (-1 + alpha2 * G_true%*%Z +  alpha3 * Z + alpha4 * X + alpha5 * errors); Y_0 <- as.numeric(Y_0)
  
  Y_next <- (-1 +  alpha1 * (G_true%*% (Y_0))  +  alpha2 * G_true%*%Z +  alpha3 * Z + alpha4 * X + alpha5 * errors); Y_next <- as.numeric(Y_next)
...
get_Y_heter <- function(Z){
  ...
  term1 <- solve(diag(length(X)) - mat1 %*% G_true) 
  term2 <-  (-diag(length(X)) %*% rep(1, length(X)) + (mat2 %*% G_true + mat3) %*% Z + mat4 %*% X + errors)
  Y <- term1 %*% term2 
...
get_Y_nonlinear <- function(Z){
  Y_0 <- (-1 + 1* G_true%*%Z + Z + X + errors) > 0
  Y_next <- Y_0 + 1
  while( max( abs(Y_0 - Y_next) ) > 0.0001){
    Y_0 <- Y_next
    Y_next <- (-1 + 1.5* G_true%*%Y_0  + 1* G_true%*%Z + Z + X + errors) >0
  }
  return(Y_0)
}
...
file_path <- file.path(repo_root, "modify", "syn_modify_misspecification_4.txt")
