...
- Experiments in Appendix (Additional Experimental Results):
  ...
  - **modify: `linear_final_attempt_bni.R`**: Bandwith selection in our 🔵 **Appendix S.5.2**. Under a fixed network realization, the main instability in our experiments arises from the unnormalized $ND-{G_2}$ estimator when the bandwidth is set to (1). In this case, the estimator minimizes an HAC objective based only on highly local network pairs, while the asymptotic expansion of this objective still contains a $\beta$-dependent bias term $R(\beta)$. Consequently, local treated-neighbor imbalance induced by a single treatment assignment can have a disproportionate impact on the solution $\hat\beta$, leading to a heavy-tailed randomization distribution with occasional extreme outliers. In comparison, using a larger bandwidth stabilizes the objective through broader neighborhood averaging, while the $\phi_0$ normalization removes the principal source of this instability more fundamentally.

  - **modify: `syn_modify_misspecification_i.R/syn_mis_nonlinear_i.R`**: Misspecification in our 🔵 **Appendix S.5.3**. Note that each round, using a different random seed induces a different misspecification scenario, which in turn leads to small fluctuations in the reported table results (you can try starting it several times if any abnormalities occur during the misspecified-network-construction). Throughout this process, our method is relatively more robust to the choice of random seed.
